...
describe('expandRruleMonth', () => {
  it(`expand rRule Month`, () => {
  ...
  it(`expand rRule MonthDay with range `, () => {
    const rRule = `DTSTART:${toRRuleDateString(
      today
  ...
  it(`expand rRule Month with range`, () => {
    const rRule = `DTSTART:${toRRuleDateString(
      today
  ...
  it(`expand rRule Month monthDay 31`, () => {
    const rRule =
  ...
  it(`expand rRule Month bySetPos with count range`, () => {
    const rRule =
    ...
    if (r) {
      ...
      expect(r.events[0].date.toISOString()).toEqual('2023-04-12T10:00:00.000Z')
      expect(r.events[0].index).toEqual(4)

      expect(r.events[1].date.toISOString()).toEqual('2023-05-10T10:00:00.000Z')
      expect(r.events[2].date.toISOString()).toEqual('2023-06-14T10:00:00.000Z')
  ...
  it(`expand rRule Month bySetPos with until Date with range`, () => {
    const rRule =
    ...
    if (r) {
      ...
      expect(r.events[0].date.toISOString()).toEqual('2023-02-08T10:00:00.000Z')
      expect(r.events[1].date.toISOString()).toEqual('2023-03-08T10:00:00.000Z')
  ...
  it(`expand rRule Month bySetPos with count`, () => {
    const rRule =
      'DTSTART:20221216T100000Z\nRRULE:FREQ=MONTHLY;INTERVAL=1;BYSETPOS=2;BYDAY=WE;COUNT=6;WKST=SU'
    ...
    if (r) {
      ...
      expect(r.events[3].date.toISOString()).toEqual('2023-04-12T10:00:00.000Z')
      expect(r.events[4].date.toISOString()).toEqual('2023-05-10T10:00:00.000Z')
      expect(r.events[5].date.toISOString()).toEqual('2023-06-14T10:00:00.000Z')
    }
  })

  it(`expand rRule Month bySetPos with until Date`, () => {
    const rRule =
  ...
  it('MONTHLY: BYMONTHDAY=31 com DTSTART em fevereiro deve manter o dia 31 (clampado) em cada mes, nao "grudar" no dia 3', () => {
    ...
    expect(r.events.length).toEqual(4)
    expect(r.events[0].date.toISOString()).toEqual('2023-02-28T10:00:00.000Z') // fevereiro clampado para 28
    expect(r.events[1].date.toISOString()).toEqual('2023-03-31T10:00:00.000Z')
    expect(r.events[2].date.toISOString()).toEqual('2023-04-30T10:00:00.000Z') // abril clampado para 30
    expect(r.events[3].date.toISOString()).toEqual('2023-05-31T10:00:00.000Z')
  })

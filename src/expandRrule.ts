...
import { Weekday, BySetPos, MonthDays, YearMonths } from './types'
import {
  getBySetPos,
  ...
  switch (r.frequency) {
    ...
    case 'WEEKLY':
      if (r.byDay && r.byDay.length > 0) {
        ...
        while (addDays(starPeriodDate, r.interval * 7) < r.startRangePeriod) {
        ...
        while (starPeriodDate < r.endRangePeriodOrUntil) {
          ...
          starPeriodDate = new Date(addDays(starPeriodDate, r.interval * 7))
        }
        ...
        dates = dates.reduce((acc: Date[], curr: Date) => {
          ...
          if (result) {
            if (
              ...
              result.getTime() <= r.endRangePeriodOrUntil.getTime()
            ) {
              acc.push(result)
            }
          }
    ...
    case 'YEARLY':
      ...
      if (r.bySetPos === 0 && r.byMonth > 0 && r.byMonthDay > 0) {
        ...
        dates = dates.map((x) =>
          setByDay(
            setByMonth(x, r.byMonth as YearMonths),
            r.byMonthDay as MonthDays
          )
        )
        r.startIndexCount = 0
        break
...
const validateAndAdjustRRule = (
  ...
  const result: IRuleExtended = {
    ...
    startRangePeriod: _dtStart > startRangePeriod ? _dtStart : startRangePeriod,
  ...
  if (!!_until && _until.getTime() < result.startRangePeriod.getTime()) {
    result.hasErrors = true
    result.errorMessages +=
      `\nInvalid recurrence rule: _startRangePeriod date (${result.startRangePeriod.toISOString()})` +
...
const setStartIndexCountAndFirstEventInRangePeriod = (
  ...
  switch (r.frequency) {
    ...
    case 'MINUTELY':
      ...
      durationFromStart = differenceInMinutes(r.startRangePeriod, _dtStart)
    ...
    case 'WEEKLY':
      ...
      break
    case 'MONTHLY':
      eventCountsFromDtStart = differenceInMonths(r.startRangePeriod, _dtStart)
      ...
      break
    case 'YEARLY':
      eventCountsFromDtStart = differenceInYears(r.startRangePeriod, _dtStart)
...
const setWeeklyFirstEvent = (r: IRuleExtended) => {
  ...
  r.firstEventInRangePeriod = getStartOfWeekWithoutChangeTime(_dtStart, r.wkst)
